...
<header class="bg-white py-4 shadow-md sticky top-0 z-10">
    <div class = "container mx-auto px-4 flex justify-between items-centre">
        ...
        <a href ="/" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounder">Home</a>
    </div>
</header>
...
<form class="container mx-auto p-5" method ="POST" enctype = "multipart/form-data">
        ...
        <label for="dropzone" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                {#if files && files.length}
                    <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">{files[0].name}</p>
                {:else}
            ...
            <input bind:files  name= "image" id="dropzone" type="file" class="hidden" />
        </label>
    </div>     
    <div class = "mb-3">
    ...
    <button type = "submit" class ="text-white bg-blue-500 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2"> Share </button>
</form>