<script>
    ...
    function resetImage() {
        if (browser) {
            ctx.putImageData(originalImageData, 0, 0); // Restore original image data
        }
    }
</script>
...
<header class="bg-opacity-75 bg-custom-white py-4 shadow-md sticky top-0 z-10">
...
<form class="container mx-auto p-5 bg-white rounded-lg shadow-md mt-6" method="POST" enctype="multipart/form-data">
...
<div class="container mx-auto p-5 bg-white rounded-lg shadow-md mt-6">
    ...
    <div class="flex justify-center mt-3">
        ...
        <button on:click={resetImage} class="bg-blue-500 hover:bg-blue-700 text-white font-medium rounded-lg text-sm px-5 py-2.5 mr-2">Original</button>
        <button on:click={saveImage} class="bg-green-500 hover:bg-green-700 text-white font-medium rounded-lg text-sm px-5 py-2.5">Save on your device</button>
    </div>
</div>
