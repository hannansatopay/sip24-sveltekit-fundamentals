...
<body class="h-page">
<header class="bg-black py-4 sticky top-0 z-10"  style="box-shadow:  0 4px 8px rgba(211, 211, 211, 0.7);">
    <div class="container mx-auto px-4 flex justify-between items-center">
        <h1 class="text-4xl text-white font-bold font-['Chiller']" >ANIMINSTA</h1>
        <a href="/" class="bg-violet-700 hover:bg-violet-800 text-white font-['Comic_Sans_MS'] py-2 px-4 rounded border border-gray-300">Home</a>
    </div>
</header>

<form class="container my-2 mx-auto p-5 font-['Comic_Sans_MS']" method="POST" enctype="multipart/form-data" action="/add-post">
    <label for="dropzone" class="mb-3 flex flex-col items-center justify-center w-full h-64 border-2 border-black border-dashed rounded-lg cursor-pointer bg-gray-400">
    ...
    <div class="mb-3">
        ...
        <input name="username" id="username" type="text" class="bg-gray-400 border border-black text-gray-900 text-sm rounded-lg block w-full p-2.5" required />
    </div>
    <div class="mb-3">
        <label for="content" class="block mb-2 text-sm font-medium text-white">Content</label>
        <textarea name="content" id="content" class="bg-gray-400 border border-black text-gray-900 text-sm rounded-lg block w-full p-2.5" required></textarea>
    </div>
    <input type="hidden" name="filter" value={selectedFilter} />
    <button type="submit" class="text-white bg-red-500 hover:bg-red-600 font-medium font-['Comic_Sans_MS'] rounded-lg text-sm px-5 py-2.5 border border-gray-300">Share</button>
</form>
</body>
