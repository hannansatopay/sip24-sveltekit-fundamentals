    ...
    function handleImage(event) {
        ...
        reader.onload = function(e) {
            img.onload = function() {
                ...
                const maxWidth = 500; // Set max width for the canvas
    ...
    <label for="dropzone" class="mb-3 flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50"> 
        <div class="flex flex-col items-center justify-center pt-5 pb-6">
            {#if files && files.length}
                ...
                <canvas id="imageCanvas" class="border border-gray-300 w-full mb-4"></canvas>
            {:else}
                <svg class="w-8 h-8 mb-4 text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"/></svg>
                <p class="text-sm text-gray-500 font-semibold">Click to upload</p>
            {/if}
        </div>
        <input bind:files name="image" id="dropzone" type="file" accept="image/png, image/jpeg" class="hidden" required on:change={handleImage} />
    </label>
    {#if files && files.length}
    <div class="flex justify-around mt-4 mb-4">
        <button type="button" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700" on:click={() => applyFilter('grayscale')}>Grayscale</button>
        <button type="button" class="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-700" on:click={() => applyFilter('sepia')}>Sepia</button>
    </div>
    {/if}
    <div class="mb-3">
