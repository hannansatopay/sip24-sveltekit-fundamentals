<script>
    export let data;
</script>

<header class="bh-white py-4 shadow-md sticky top-0 z-10">
    <div class="container mx-auto px-4 flex justify-between items-center">
        <h1 class="text-2xl font-bold font-['Comic_Sans_Ms']">InstaLab</h1>
        <a href="/add-post" class="bg-blue-500 text-white font-bold px-4 py-2 rounded hover:bg-blue-700">Add Post</a>
    </div>
</header>

<div class="container mx-auto md:my-5">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <!-- Post Template -->
         {#each data.posts as post }
            <div>
                <img src="data:image;base64, {post.image}" alt="post-pic" class="w-full">
                <div class="py-2 text-sm">
                    <span class="font-bold">{post.username}</span>
                    <span class="text-gray-700">{post.content}</span>
                </div>
            </div>
         {/each}
    </div>
</div>